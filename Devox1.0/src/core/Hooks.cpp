...
#include "../Devox1.0/repos/minhook/MinHook.h"
...
HRESULT __stdcall hook_present(IDXGISwapChain* swap_chain, UINT sync_interval, UINT flags) {
	...
	return hooks::original_present(interfaces::dx11_csereinput->swap_chain, sync_interval, flags);
}
...
HRESULT __stdcall hook_resize_buffers(IDXGISwapChain* swap_chain, UINT buffer_count, UINT width, UINT height, DXGI_FORMAT new_format, UINT swap_chain_flags) {
	const HRESULT result = hooks::original_resize_buffers(swap_chain, buffer_count, width, height, new_format, swap_chain_flags);
...
bool __fastcall hook_mouse_input_enabled(sdk::interface_cs2_input* csgo_input) {
	return globals::menu_opened ? false : hooks::original_mouse_input_enabled(csgo_input);
}


void* __fastcall hook_set_relative_mouse_mode(sdk::interface_input_rendszer* input_rendszer, bool enabled) {
	globals::relative_mouse_mode = enabled;
	return hooks::original_set_relative_mouse_mode(input_rendszer, globals::menu_opened ? false : enabled);
}
...
namespace hooks {
	void create() {
		...
		if (MH_CreateHook(
			sdk::virtual_function_get<void*, 8>(interfaces::dx11_csereinput->swap_chain), 
			...
			if (MH_CreateHook(
			sdk::virtual_function_get<void*, 13>(interfaces::dx11_csereinput->swap_chain),
			&hook_resize_buffers, reinterpret_cast<void**>(&original_resize_buffers)) != MH_OK) {
			throw std::runtime_error("Failed to create hook for IDXGISwapChain::ResizeBuffers.");
		}
...

			{
				...
				if (FAILED(interfaces::d3d11_device->QueryInterface(
					...
					throw std::runtime_error("Failed to get IDXGIDevice from ID3D11Device.");
				}
				...
				if (FAILED(dxgi_adapter->GetParent(__uuidof(IDXGIFactory), reinterpret_cast<void**>(&dxgi_factory)))) {
				...
				if (MH_CreateHook(
					sdk::virtual_function_get<void*, 10>(dxgi_factory),
					&hook_create_swap_chain, reinterpret_cast<void**>(&original_create_swap_chain)) != MH_OK) {
					dxgi_factory->Release();
					dxgi_adapter->Release();
					dxgi_device->Release();
					throw std::runtime_error("Failed to create hook for IDXGIFactory::CreateSwapChain.");
				}
			...
			if (MH_CreateHook(
				sdk::virtual_function_get<void*, 15>(interfaces::cs2_input),
				&hook_mouse_input_enabled,
				reinterpret_cast<void**>(&original_mouse_input_enabled)) != MH_OK) {
				throw std::runtime_error("Failed to create hook for cs2_input::mouse_input_enabled.");
			}


			if (MH_CreateHook(
				sdk::virtual_function_get<void*, 76>(interfaces::input_rendszer),
				&hook_mouse_input_enabled, reinterpret_cast<void**>(&original_set_relative_mouse_mode)) != MH_OK) {
				throw std::runtime_error("Failed to create hook for cs2_input::mouse_input_enabled.");
			}


		if (MH_EnableHook(MH_ALL_HOOKS) != MH_OK) {
			throw std::runtime_error("Failed to enable MinHook.");
		}
	}
